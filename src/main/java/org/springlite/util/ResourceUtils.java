...
public abstract class ResourceUtils  {
     ...
     */
    public static String getFileDirectory(URL url) throws MalformedURLException{
        if(!isFileURL(url)){
            throw new MalformedURLException("URL is not file url");
        }
        String filePath = url.getPath();
        int last = filePath.lastIndexOf(File.separator);
        if(last <=0 || last >= filePath.length()){
            throw new MalformedURLException("File path is invalid");
        }
